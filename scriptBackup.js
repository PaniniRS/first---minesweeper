...
/*
#TODO: 
  2. Make the timer functional

  #GAMEOVER:
    4. Make the game end when you have flagged all the bombs
    5. Make the game end when you have opened all the tiles
    9. Show a modal/popup when the game is over with a restart button
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "R" || e.key === "r") resetGameModal();
  if (e.key === "Escape") closeModal();
});
appendClose.addEventListener("click", closeModal);
document.querySelector(`.retry`).addEventListener(`click`, resetGameModal);
